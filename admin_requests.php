...
<form method="POST" id="requestForm">
    <div id="bulkBar" class="bulk-actions" style="display: none; margin-bottom: 20px;">
        ...
        <button type="submit" name="bulk_action" value="approve" class="btn btn-mini" style="background: #10B981; color: white; border: none; padding: 8px 16px; border-radius: 8px;">Approve All</button>
        <button type="submit" name="bulk_action" value="reject" class="btn btn-mini" style="background: #EF4444; color: white; border: none; padding: 8px 16px; border-radius: 8px;">Reject All</button>
    </div>
    ...
    <div style="background: white; border-radius: 16px; box-shadow: var(--shadow-sm); overflow: hidden;">
        <table class="admin-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><input type="checkbox" name="selected_requests[]" value="<?php echo $r['id']; ?>" class="admin-checkbox" onchange="updateBulkBar()"></td>
                    <td>
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="width: 36px; height: 36px; border-radius: 50%; background: #F3F4F6; display: flex; align-items: center; justify-content: center; font-weight: 700; color: #6B7280; font-size: 0.8rem;">
                                <?php echo strtoupper(substr($r['student_name'], 0, 1)); ?>
                            </div>
                            <div>
                                <div style="font-weight: 700; color: #111827; font-size: 0.9rem;"><?php echo htmlspecialchars($r['student_name']); ?></div>
                                <div style="font-size: 0.75rem; color: #6B7280;"><?php echo htmlspecialchars($r['student_email']); ?></div>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div style="font-weight: 600; color: #374151;"><?php echo htmlspecialchars($r['trip_title']); ?></div>
                    </td>
                    <td><span style="font-size: 0.8rem; color: #9CA3AF;"><?php echo date('M d, Y H:i', strtotime($r['request_date'])); ?></span></td>
                    <td>
                        <?php 
                            $st = $r['status'];
                    ...
                    <td>
                        <div style="display: flex; gap: 8px;">
                            <?php if($r['status'] == 'pending'): ?>
                                <a href="?action=approve&rid=<?php echo $r['id']; ?>" class="btn-icon" style="background: #D1FAE5; color: #065F46;" title="Approve"><i class="ri-check-line"></i></a>
                                <a href="?action=reject&rid=<?php echo $r['id']; ?>" class="btn-icon" style="background: #FEE2E2; color: #B91C1C;" title="Reject"><i class="ri-close-line"></i></a>
                            <?php else: ?>
                                <a href="?action=approve&rid=<?php echo $r['id']; ?>" class="btn-icon" style="background: #F3F4F6; color: #6B7280; font-size: 0.7rem; width: auto; padding: 0 8px;">Re-Approve</a>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
