...
if ($role == 'admin')
    $has_access = true;
if ($role == 'tripmaker' && $trip['created_by'] == $user_id)
    $has_access = true;
...
foreach ($included as $item)
    if (trim($item))
        echo "<li style='margin-bottom: 5px; display: flex; align-items: flex-start; gap: 8px;'><i class='ri-check-line' style='margin-top: 3px;'></i> <span>" . htmlspecialchars(trim($item)) . "</span></li>";
?>
                </ul>
            </div>
...
foreach ($not_included as $item)
    if (trim($item))
        echo "<li style='margin-bottom: 5px; display: flex; align-items: flex-start; gap: 8px;'><i class='ri-close-line' style='margin-top: 3px;'></i> <span>" . htmlspecialchars(trim($item)) . "</span></li>";
?>
                </ul>
            </div>
        </div>
...
?>
    ...
    if ($c_result->num_rows > 0) {
        while ($msg = $c_result->fetch_assoc()) {
            ...
            if (!$is_me)
                echo '<strong style="display:block; font-size:0.7rem; margin-bottom:4px; opacity: 0.8;">' . htmlspecialchars($msg['name']) . '</strong>';
...
?> Days</strong>
    ...
    elseif ($role == 'student'): ?>
        ...
        if ($trip['max_participants'] > 0) {
            ...
            if ($curr >= $trip['max_participants']) {
                $can_join = false;
                $error_msg = "Trip is Full";
            }
        }
        if ($trip['status'] != 'active') {
            $can_join = false;
            $error_msg = "Booking Closed";
        }
?>
...
if ($revs->num_rows > 0) {
    while ($rev = $revs->fetch_assoc()) {
        ...
        if (!empty($rev['review_text']))
            echo '<p style="font-size: 0.85rem; color: var(--text-light); margin-bottom: 4px;">' . htmlspecialchars($rev['review_text']) . '</p>';
